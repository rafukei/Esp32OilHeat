...
void webServerTask(void *parameter) {
    ...
    server.on("/control", HTTP_GET, []() {
        ...
        if (server.hasArg("burner")) {
            ...
            // The relayTask will decide based on manual/auto mode
...
void setup() {
    ...
    // Relay control (high priority) - NOW WITH MANUAL MODE PRIORITY
    xTaskCreatePinnedToCore(
        relayTask,
        "Relay",
        TASK_STACK_SMALL,
    ...
    // Control logic (YOUR ORIGINAL VERSION - UNCHANGED)
    xTaskCreatePinnedToCore(
        controlTask,
        "Control",
        TASK_STACK_MEDIUM,
    ...
    Serial.println("\nMANUAL MODE PRIORITY: Manual commands override all automatic control");
    Serial.println("Emergency stop works in all modes (highest priority)");
    Serial.println("NEW FEATURE: 10-minute delay after heating turns off");
