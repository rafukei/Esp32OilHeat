...
void loadSystemSettings() {
  ...
  if (xSemaphoreTake(xSettingsMutex, pdMS_TO_TICKS(100)) == pdTRUE) {
    ...
    systemState.settings.hysteresis = preferences.getFloat("hysteresis", DEFAULT_HYSTERESIS);
...
/**
 * Control logic task
 * Executes automatic burner control logic
 * Checks temperatures and updates burner state based on hysteresis
 */
void controlTask(void *parameter) {
  ...
  while (1) {
    ...
    if (currentTime - lastControlTime >= 1000) {
      ...
      // If errors or heating disabled, no automatic control
      if (anyFault || heatingDisabled) {
        canControl = false;
      }
      
      // Get required temperatures safely
      if (lockSensorAndTemperature()) {
        ...
        // Manual mode prevents automatic control
        if (manualMode) {
          canControl = false;
        }
      ...
      // Execute automatic control if conditions met AND targetTemp > 0
      if (canControl && targetTemp > 0) {
        ...
        if (!currentBurnerState && waterTempToUse < (targetTemp - hysteresis)) {
          // Burner off, water too cold -> turn on
          newBurnerState = true;
        } 
        else if (currentBurnerState && waterTempToUse > (targetTemp + hysteresis)) {
          // Burner on, water too hot -> turn off
          newBurnerState = false;
        }
        
        // If state changed, update it and send command to relay
...
void setup() {
   ...
   * Load temperature curve and system settings
   */
  loadSystemSettings();
  Serial.println("System settings loaded from flash memory");
